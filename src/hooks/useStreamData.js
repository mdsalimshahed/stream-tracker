...
export function useStreamData(notify) {
  ...
  // --- Automatic Global Storage Logic w/ Intelligent Eviction ---
  useEffect(() => {
    let currentData = JSON.parse(JSON.stringify(streamData));
    
    try {
      localStorage.setItem('streamManagerData', JSON.stringify(currentData));
    } catch (e) {
      ...
      if (isQuotaError) {
        ...
        // 1. Sort games by oldest stream activity first so we evict from older content
        const sortedGameIds = Object.keys(currentData).sort((a, b) => {
          const getLatest = (g) => {
            let latest = 0;
            Object.values(g.cycles || {}).forEach(c => {
              if (c.timestamps?.length) {
                const lastTs = c.timestamps[c.timestamps.length - 1];
                const tsStr = lastTs.date || lastTs.startTime || lastTs;
                const ts = new Date(tsStr).getTime();
                if (ts > latest && !isNaN(ts)) latest = ts;
              }
            });
            return latest;
          };
          return getLatest(currentData[a]) - getLatest(currentData[b]);
        });

        // 2. Find EXACTLY ONE base64 image to delete across the entire library
        for (const gId of sortedGameIds) {
          const game = currentData[gId];
          
          // Try removing a non-cover thumbnail first
          if (game.thumbnail_urls && game.thumbnail_urls.length > 0) {
            const b64Idx = game.thumbnail_urls.findIndex(img => img.startsWith('data:image') && img !== game.cover_image);
            if (b64Idx !== -1) {
              game.thumbnail_urls.splice(b64Idx, 1);
              victimFound = true;
              break; // Stop immediately after finding ONE image
            }
          }
          
          // If no non-cover local images, try removing the cover image if it's base64
          if (!victimFound && game.cover_image && game.cover_image.startsWith('data:image')) {
            const oldCover = game.cover_image;
            game.cover_image = game.thumbnail_urls?.find(img => img !== oldCover) || 'https://placehold.co/600x400/1e293b/475569?text=Cover';
            
            const b64Idx = game.thumbnail_urls?.indexOf(oldCover);
            if (b64Idx !== -1 && game.thumbnail_urls) {
              game.thumbnail_urls.splice(b64Idx, 1);
            }
            victimFound = true;
            break; // Stop immediately after finding ONE image
        ...
        // 3. Retry the save once with the single evicted image
        if (victimFound) {
          try {
            localStorage.setItem('streamManagerData', JSON.stringify(currentData));
            notify('Memory limit reached: A single older image was removed to save new stream data.', 'info');
            setStreamData(currentData); // Sync state with the evicted data
          } catch (retryError) {
            console.error("Still exceeding quota after evicting one image.", retryError);
            notify('Storage completely full! Please delete some games manually.', 'error');
          }
        } else {
          console.error("Browser storage completely full. No local images left to evict.");
          notify('Storage completely full! Please delete some games manually.', 'error');
        }
      } else {
  ...
  const handleManualSync = async () => {
    if (isSyncing) return;
    setIsSyncing(true);
    notify('Starting YouTube playlist sync...', 'info');

    try {
      const dataCopy = JSON.parse(JSON.stringify(streamData));
      let changed = false;

      // YouTube Data syncing (Playlist data only)
      for (const [id, game] of Object.entries(dataCopy)) {
      ...
      if (changed) {
        ...
        notify('YouTube Playlists sync completed!', 'success');
      } else {
        notify('Playlists are already up to date.', 'info');
      }
    } catch (error) {
