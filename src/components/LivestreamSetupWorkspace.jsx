...
import { X, ChevronLeft, ImagePlus, Globe, Plus, Save, Loader2, Trash2, Bold, Italic, Image as ImageIcon, TextSelect } from 'lucide-react';
import ThumbnailCanvas from './ThumbnailCanvas';
import SeoGeneratorPanel from './SeoGeneratorPanel';
...
export default function LivestreamSetupWorkspace({ 
  ...
  const handleAddGoogleFont = async () => {
    ...
    if (!urlString.includes('fonts.googleapis.com/css2')) {
       onNotify('Invalid Google Fonts code or URL', 'error'); return;
    }
    
    try {
      const parsedUrl = new URL(urlString);
      const families = parsedUrl.searchParams.getAll('family');
      if (families.length === 0) { onNotify('No font families found in URL', 'error'); return; }
      ...
      if (addedCount > 0) {
         ...
         if (!document.querySelector(`link[href="${urlString}"]`)) {
            const link = document.createElement('link'); link.rel = 'stylesheet'; link.href = urlString;
            link.onload = async () => { try { await document.fonts.load(`16px "${newFamilies[0]}"`); } catch(e) {} setCF(newFamilies[0]); };
            document.head.appendChild(link);
         } else { setCF(newFamilies[0]); }
         onNotify(`Added ${addedCount} new font(s)`, 'success');
      } else { onNotify('Font(s) already in gallery', 'info'); }
      setEmbedCode('');
    } catch (e) { onNotify('Failed to parse URL', 'error'); }
  };
    ...
    <>
      <div className="fixed inset-0 z-50 flex bg-black overflow-hidden">
        <div className="w-full h-full bg-neutral-900 flex flex-col">
          ...
          {/* Workspace Header */}
          <div className="flex flex-col sm:flex-row justify-between sm:items-center px-4 sm:px-6 py-3 border-b border-white/10 bg-neutral-900/80 backdrop-blur-sm shrink-0 gap-3">
          ...
          {activeTab === 'thumbnail' && (
            <div className="flex-1 flex flex-col lg:flex-row overflow-y-auto lg:overflow-hidden custom-scrollbar animate-in fade-in duration-300">
              ...
              <div className="flex-none lg:flex-1 flex flex-col bg-black min-h-[300px] shrink-0 lg:min-h-0 overflow-hidden">
                <div className="mx-4 sm:mx-8 mt-4 p-2 sm:p-3 bg-white/10 rounded-lg text-center cursor-pointer hover:bg-white/20 transition" onClick={() => handleCopy(title, false, 'Title Copied!')}>
                ...
                <div className="flex-1 relative overflow-hidden">
                  {selImg ? (
                    <ThumbnailCanvas canvasRef={workspaceCanvasRef} bgImageUrl={selImg} gameName={game.game_name} cycleName={cycleDisplayName} streamCount={nC} config={config} customFont={cF} />
                  ) : (
        ...
        {confirmDialog && (
          <ConfirmBanner title={confirmDialog.title} message={confirmDialog.message} onConfirm={confirmDialog.onConfirm} onCancel={() => setConfirmDialog(null)} />
        )}
        {confirmFontDialog && (
          <ConfirmBanner title={confirmFontDialog.title} message={confirmFontDialog.message} onConfirm={confirmFontDialog.onConfirm} onCancel={() => setConfirmFontDialog(null)} />
        )}
      </div>
    </>
  );
